...
@interface KSYShortVideoPlayVC ()<UITableViewDataSource,UITableViewDelegate,KSYMediaPlayerDelegate,UITextFieldDelegate>
{
    KSYPlayer *_player;
    NSURL *videoUrl;
    //播放器状态
...
- (void)initPlayerWithLowTimelagType:(BOOL)isLowTimeType {
    ...
    _player = [[KSYPlayer alloc] initWithURL:videoUrl];
    _player.delegate=self;
    _player.shouldAutoplay = YES;
    [_player prepareToPlay];
    [self.view addSubview:_player.videoView];
    //设置播放器播放视图的大小
    _player.videoView.frame = CGRectMake(0,64,self.view.width,(self.view.height-64)/2);
...
{
    self.kShortTopView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, _player.videoView.width, 40)];
    [_player.videoView addSubview:self.kShortTopView];
...
{
    self.kShortBottomView=[[UIView alloc]initWithFrame:CGRectMake(0, _player.videoView.height-40, _player.videoView.width, 40)];
    [_player.videoView addSubview:self.kShortBottomView];
...
- (void)addBellowPart
{
    ...
    CGFloat backgroundVieWidth=_player.videoView.width;
    CGFloat backgroundViewHeight=THESCREENHEIGHT-_player.videoView.bottom;
...
- (void)kShortShowLoading {
    if (_kShortLodingView == nil) {
        _kShortLodingView = [[UIView alloc] initWithFrame:_player.videoView.bounds];
        _kShortLodingView.backgroundColor = [UIColor clearColor];
        [_player.videoView addSubview:_kShortLodingView];
...
- (void)kShortShowError {
    if (_kShortErrorView == nil) {
        _kShortErrorView = [[UIView alloc] initWithFrame:_player.videoView.bounds];
        _kShortErrorView.backgroundColor = [UIColor clearColor];
        [_player.videoView addSubview:_kShortErrorView];
...
- (void)reSetLoadingViewFrame
{
    if (!_kShortLodingView.hidden) {
        _kShortLodingView.frame = _player.videoView.bounds;
    ...
    if (!_kShortErrorView.hidden) {
        _kShortErrorView.frame = _player.videoView.bounds;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (fabs(deltaX) < fabs(deltaY)) {//如果是纵向滑动
    ...
    else if (curPoint.y>64&&curPoint.y<_player.videoView.bottom&&duration > 0 && (_gestureType == kKSYUnknown || _gestureType == kKSYProgress)) {
        ...
        if (fabs(deltaX) > fabs(deltaY)) {
            ...
            CGFloat totalWidth=_player.videoView.width;
